package io.github.treesitter.jtreesitter.sln;

import java.lang.foreign.*;

public final class TreeSitterSln {
    ...
    private static final TreeSitterSln INSTANCE = new TreeSitterSln();

    private final Arena arena = Arena.ofAuto();
    private volatile SymbolLookup lookup = null;

    private TreeSitterSln() {}
    ...
    public static MemorySegment language(SymbolLookup lookup) {
        return call(lookup, "tree_sitter_sln");
    }

    private SymbolLookup findLibrary() {
        try {
            var library = System.mapLibraryName("tree-sitter-sln");
            return SymbolLookup.libraryLookup(library, arena);
        } catch (IllegalArgumentException ex1) {
            try {
                System.loadLibrary("tree-sitter-sln");
